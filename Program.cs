...
namespace wheel_of_fortune
{
    ...
    class Game
    {
        ...
        string _chosenProverb; // The chosen proverb in string
        char[] _chosenProverbArray; // An array of underlines which changes according to the guessed characters
        int[] _playerScores;
        ...
        int _outGuess(char Guess, int Player)
        {
            ...
            Console.WriteLine("Player {0} spinned value {1} HUF", Player + 1, _prize);
            _multiplier = 0;
            ...
            Console.WriteLine("\nPlayer {0} earns {1}*{2} = {3} HUF", Player + 1, _multiplier, _prize, _multiplier * _prize);
        ...
        int _outGuess(string Guess, int Player)
        {
            ...
            if (_multiplier == _chosenProverb.Length)
            {
                ...
                Console.WriteLine("Player {0} guessed the sentence:\n{1}", Player + 1, _chosenProverb);
                Console.WriteLine("Player {0} wins {1}*{2} HUF", Player + 1, _multiplier, _prize);
                return _prize * _multiplier;
            }
            Console.WriteLine("Player {0}'s guess wasn't right.", Player + 1);
            return 0;
        }
        ...
        /// The user is given the option to choose the number of players.
        /// </summary>
        void _init()
        {
        ...
        bool _checkForGuessed(char Guess)
        {
            int i = 0;
            while (i < _chosenProverbArray.Length && _chosenProverbArray[i] != Guess)
            {
                i++;
            }
            return i < _chosenProverbArray.Length;
        }
        ...
        public void GameLoop()
        {
            ...
            do
            {
                ...
                _guess = ' ';
                //Player loop
                while (_player < _playerScores.Length && _compare() == false)
                {
                    //Individual player loop, when guesses are correct, it repeats
                    do
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                else if (_checkForGuessed(_guess))
                                {
                                    Console.WriteLine("You can't guess a letter that was already guessed, try again!");
                                    _guess = ' ';
                                    _win = -1;
                                    continue;
                                }
            ...
            Console.WriteLine("Player {0} won the entire game and took home {1} HUF", _player + 1, _playerScores[_player]);
            // Game restart option
            string _reset = "";
